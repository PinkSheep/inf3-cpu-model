/*
This javascript code represents a simple CPU-model (without any fancy stuff like cache, multicore)
As input it recieves operations represented by assembler-like instructions which are then transformed into 16-byte binary strings
...
window.onload = function() { 
	...
	// Eventhandling for op-code input textarea
	...
	//Eventhandling for memory input textarea
	...
	//Run Modus - all the operation stored in the opCodeArray are executed at once
	$("#run").click(function(){
		//loop runs as long as there are still operations left to execute
	...
	//Step Modus - only one operation is executed when pressing the step button
	$("#step").click(function(){
		callOpCodeFunction();
		refreshFrontend(akku,reg1,reg2,reg3,befehlpointer,befehlszaehler,speicherArr,carryflag);
...
function parseOpcode(mnemonic){
	//transforms assembler-like instructions into 16-byte binary strings
	switch(true){
